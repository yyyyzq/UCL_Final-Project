-------------------------------------------------------------------------------
-- Title      : Single Polarization System with Constellation Recorder
-- Project    : CHOICE
-------------------------------------------------------------------------------
-- File       : system.vhdl
...
-- channel and receiver with constellation recording capability.
--
-------------------------------------------------------------------------------
...
architecture arch of system is
  ...
  -- 添加内部信号，用于强制覆盖const_done
  signal const_done_internal : std_logic;
...
begin
  ...
  receiver_inst : component receiver
    ...
    port map (clk        => clk,
              ...
              const_done => const_done_internal);  -- 连接到内部信号

  -- 强制const_done永远为'0'，忽略receiver的输出
  -- 正确连接const_done信号
const_done <= const_done_internal;  -- 使用真实的完成信号  -- 这里！永远不完成！
