...
file iq_file : text open write_mode is "constellation_output.txt";
    ...
    if rst = '1' then
      ...
      if valid_in = '1' then
        ...
        for p in 0 to PAR-1 loop
            i_val := to_integer(signed(i_in((p+1)*WIDTH -1 downto p*WIDTH)));
            q_val := to_integer(signed(q_in((p+1)*WIDTH -1 downto p*WIDTH)));
            write(L, i_val);
            write(L, string'(" "));
            write(L, q_val);
            writeline(iq_file, L);
